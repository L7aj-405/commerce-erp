...
class FinanceSituationPdfExport
{
    ...
    private function section(Organization $organization, FinancePeriod $period, ?Store $store): array
    {
        ...
        return [
            ...
            'ventes_nettes' => $this->format->money($situation['ventes_nettes']),
            'facturation' => $this->format->money($situation['facturation']),
            'facturation_brute' => $this->format->money($situation['facturation_brute']),
            'avoirs' => $this->format->money($situation['avoirs']),
            'encaissements' => $this->format->money($situation['encaissements']),
            'remboursements' => $this->format->money($situation['remboursements']),
            'net_encaisse' => $this->format->money($situation['net_encaisse']),
            'creances_debut' => $this->format->money($situation['creances_debut']),
            'creances_fin' => $this->format->money($situation['creances_fin']),
            'obligations_remboursement' => $this->format->money($situation['obligations_remboursement']),
            'position_nette_debut' => $this->format->money($situation['position_nette_debut']),
            'position_nette_fin' => $this->format->money($situation['position_nette_fin']),
            'has_variance' => Decimal::compare($situation['customer_position']['variance'], '0.0000') !== 0,
            'variance' => $this->format->money($situation['customer_position']['variance']),
